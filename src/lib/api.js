...
export async function fetchAdminData() {
  const [
    ...
    optional(t("invoices").select("*, customers(*), invoice_items(*)").order("billing_at", { ascending: false }).limit(2500), []),
    optional(t("customers").select("*").order("last_visit_at", { ascending: false }).limit(2000), []),
    optional(t("transactions").select("*").order("created_at", { ascending: false }).limit(2500), []),
    optional(t("report_logs").select("*").order("created_at", { ascending: false }).limit(500), []),
    ...
    optional(t("cash_register").select("*").order("date", { ascending: false }).limit(365), []),
    optional(t("attendance_logs").select("*").order("created_at", { ascending: false }).limit(1000), []),
    ...
    optional(t("expenses").select("*").order("date", { ascending: false }).limit(2000), []),
    optional(t("expense_categories").select("*").order("sort_order"), []),
    optional(t("invoice_tip_splits").select("*").order("created_at", { ascending: false }).limit(2500), []),
